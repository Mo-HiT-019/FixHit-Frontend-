...
const VerifyResetOtpTech = () => {
  ...
  useEffect(() => {
  if (!email) {
    navigate("technician/forgot-password"); 
  }
}, [email]);
  ...
  const handleVerify = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await axios.post("/technicians/verify-reset-otp", { email, otp });
      navigate("technicians/reset-password", { state: { email } });
    } catch (err: any) {
...
export default VerifyResetOtpTech;